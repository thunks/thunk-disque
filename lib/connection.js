...
class Connection {
  constructor (disque, id) {
    ...
    this.isLocalHost = id.indexOf(LOCAL_HOST) >= 0
  ...
  reconnecting () {
    ...
    if (disqueState.connection === this) {
      for (let id in disqueState.pool) {
        if (id !== this.id) {
  ...
  checkConnection () {
    ...
    disqueState.thunk((callback) => {
      ...
      if (disqueState.connected) this.flushQueue()
      else {
        disqueState.connected = true
        disqueState.connection = this
  ...
  createResp () {
    ...
    return new Resp({returnBuffers: this.returnBuffers})
      .on('drain', () => this.flushQueue())
      .on('error', (error) => {
        this.rescuePending()
  ...
  tryRemove (error, tryEnd) {
    ...
    for (let i = 0; i < disqueState.nodes.length; i++) {
    ...
    if (tryEnd && !disqueState.nodes.length) disque.clientEnd(error)
    else {
      // auto connect a new node
      connectNewNode(disque)
      // dispatch commands again
      process.nextTick(() => dispatchCommands(disque))
    }
  }

  flushQueue () {
    if (!this.connected || !this.queue.length || this.pendingQueue.length > 256) return this
    let maxPipeline = 4096
    this.socket.cork()
    while (maxPipeline-- && this.queue.length) this.writeCommand(this.queue.shift())
    this.socket.uncork()
    return this
  }
...
function updateNodes (connection) {
  ...
  return disque._disqueState.thunk(function (callback) {
    let command = createCommand(disque, 'hello', [], function (error, res) {
      ...
      if (disqueState.nodes.length < 2 && disqueState.connections.length < 2) {
...
function meetNodes (src, dest) {
  ...
  let ip = dest.id.split(':')
  return disque._disqueState.thunk(function (callback) {
    let command = createCommand(disque, 'cluster', ['meet', ip[0], ip[1]], function (error, res) {
      if (error) return callback(error)
...
function dispatchCommands (disque, command) {
  ...
  if (commandQueue.length) {
    ...
    tool.each(connections, (connection) => connection.flushQueue())
  }
...
function dispatchCommand (disqueState, command) {
  ...
  if (!connection || connection.ended) {
    process.nextTick(() => command.callback(new Error('connection not exist')))
    return
  }
...
;[
  ...
  'show',
  'working'
].forEach((command) => isJobCommand[command] = true)
